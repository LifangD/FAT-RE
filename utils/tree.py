...
def head_to_tree(head, tokens, len_, prune, subj_pos, obj_pos):
    ...
    if prune < 0:
        nodes = [Tree() for _ in head] # every node is a tree

        for i in range(len(nodes)):
            h = head[i]  # the node's head
            nodes[i].idx = i # the node's index
            nodes[i].dist = -1 # just a filler
            if h == 0:
                root = nodes[i]
            else:
                nodes[h-1].add_child(nodes[i]) # build a tree
    else:
        ...
        path_nodes = subj_ancestors.union(obj_ancestors).difference(cas) # not clear
        path_nodes.add(lca)

        # compute distance to path_nodes
        dist = [-1 if i not in path_nodes else 0 for i in range(len_)] # only self dis is 0

        for i in range(len_):
            if dist[i] < 0:
                stack = [i] # for every node
                while stack[-1] >= 0 and stack[-1] not in path_nodes: # the last one  not in path, use stack to store its head
...
def tree_to_adj(sent_len, tree, directed=True, self_loop=False):
    ...
    return ret
def cont_tree_to_adj(sent_len, tree, directed=True, self_loop=False):
    """
    Convert a tree object to an (numpy) adjacency matrix.
    """
    ret = np.zeros((sent_len, sent_len), dtype=np.float32)
    ...
    if not directed:
     ...
     # adding the context info
    window = 2
    for i in idx:
        if i+window < sent_len:
            ret[i, i:i+window] = 1
        else:
            ret[i,i:] = 1

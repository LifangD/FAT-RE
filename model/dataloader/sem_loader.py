...
class DataLoader(object):
    ...
    def preprocess(self, data, vocab, opt,):
        ...
        for d in data:
            ...
            op_relation = self.op_label2id[d['relation']]
            pc_mask = None
            if self.over_sampling and relation!=0 : # over-sampling in training
                processed += 2*[(tokens, pos, ner, deprel, head, subj_positions,obj_positions,subj_positions2,obj_positions2,subj_type, obj_type, pc_mask,relation)]
            else:
                processed += [(tokens, pos, ner, deprel, head, subj_positions,obj_positions,subj_positions2,obj_positions2,subj_type, obj_type, pc_mask,relation)]
                # if op_relation!=0 and not self.eval:
                #     processed += [(tokens, pos, ner, deprel, head, obj_positions, subj_positions, obj_positions2,
                #                         subj_positions2, obj_type, subj_type, pc_mask,op_relation)]

        return processed

    def preprocess_v2(self, data, vocab, opt, ):
        """ Preprocess the data and convert to ids. """
        processed = []
        for d in data:
            tokens = list (d['token'])
            ...
            res+=[subj_type,obj_type,pcmask,relation]
    ...
    def __getitem__(self, key):
        ...
        if "processed_semeval" in self.opt['data_dir']:
            deprel = None
            head = None
            subj_type = None
            obj_type  = None
            ...
            pcmask = torch.LongTensor (batch[11])
        ...
        return (words, masks, pos, ner, deprel, head, subj_positions, obj_positions,subj_positions2, obj_positions2,  subj_type, obj_type, pcmask, rels,orig_idx,)
